...
parser.add_argument('--input-path', default='./preprocessing/mimic-cxr-radgraph-sentence-preprocessed.csv',
    ...
    def __getitem__(self, idx):
        ...
        for index, row in df_sid.iterrows():
            try:
                ...
                # binary label for a landmark, observation pair
                curr_val = landmark_observation_adj_mtx[landmark_idx, observation_idx]

                # prognosis mentions for a landmark, observation pair
                if isinstance(row.obs_modify, str):
                    new_word = extract_prognosis_word(row.obs_modify)
                    if new_word != '':
                        # if key alreay exists, then concatenate the strings
                        if (landmark_idx, observation_idx) in prognosis_dict.keys():
                            existing_words = prognosis_dict[(landmark_idx, observation_idx)].split('|')
                            if new_word not in existing_words:
                                prognosis_dict[(landmark_idx, observation_idx)] = prognosis_dict[(
                                landmark_idx, observation_idx)] + '|' + new_word
                        else:
                            prognosis_dict[(landmark_idx, observation_idx)] = new_word
                ...
                pass
        return sid, landmark_observation_adj_mtx, str(prognosis_dict)
...
def extract_prognosis_word(obs_modify):
    """Extract prognosis word from the input string."""

    modify_dp_words = ['unchanged', 'constant', 'similar', 'stable',
                       'increased', 'increase', 'increasing',
                       'worsening', 'worse', 'worsened',
                       'greater', 'larger', 'exaggerated',
                       'progression', 'progressed', 'progressive',
                       'development', 'developing', 'continued',
                       'new',
                       'smaller', 'reduced',
                       'decreased', 'decrease', 'decreasing',
                       'improved', 'improvement', 'improving', 'better',
                       'resolved', 'healed', 'resolving']

    words = obs_modify.split('|')
    outputs = []
    for w in words:
        if (w in modify_dp_words) & (w not in outputs):
            outputs.append(w)
    return '|'.join(outputs)
...
def create_adj_matrix(args):
    ...
    for index, data in tqdm(enumerate(loader), total=len(loader)):
        sid, landmark_observation_adj_mtx, prognosis_dict = data
        sid_lst.append(sid)
        adj_mtx_lst.append(landmark_observation_adj_mtx)
        prognosis_lst.append(prognosis_dict)
    ...
    full_prognosis_dict = np.concatenate(prognosis_lst, axis=0)
    dict_adj_mtx = {}
    for i in trange(len(full_sids)):
        sid = full_sids[i]
        dict_adj_mtx[sid] = {}
        dict_adj_mtx[sid]['adj_mtx'] = full_adj_mtx[i]
        dict_adj_mtx[sid]['prognosis'] = full_prognosis_dict[i]
    ...
    np.save('./preprocessing/landmark_observation_sids.npy',
            full_sids)
    print('landmark_observation_sids.npy has been saved!')
    np.save('./preprocessing/landmark_observation_adj_mtx.npy',
            full_adj_mtx)
    print('landmark_observation_sids.npy has been saved!')
    np.save('./preprocessing/prognosis_dict.npy',
            full_prognosis_dict)
    print('prognosis_dict.npy has been saved!')

    with open('./preprocessing/mimic-cxr-adj-mtx-prognosis.pickle', 'wb') as handle:
        pickle.dump(dict_adj_mtx, handle, protocol=pickle.HIGHEST_PROTOCOL)
    print('mimic-cxr-adj-mtx-prognosis.pickle has been saved!')
